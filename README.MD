...
MyScheduler is a Spring Boot application that implements the outbox pattern for reliable message publishing to Kafka. It
ensures that messages are published to Kafka exactly once, even in the presence of failures.
...
- `src/main/java/io/msdousti/myscheduler/service/ScheduledOutboxPublication.kt`: Core service that implements the outbox
  pattern
- `src/main/java/io/msdousti/myscheduler/service/KafkaPublisher.kt`: Service for publishing messages to Kafka
- `src/main/java/io/msdousti/myscheduler/repo/OutboxRepository.kt`: Repository for accessing the outbox table
